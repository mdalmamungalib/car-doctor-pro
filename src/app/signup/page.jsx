    ...
    try {
      ...
      if (res.status === 201) {
        ...
        Swal.fire({
          ...
          confirmButtonText: "Login", // Custom button text
          customClass: {
            popup: "bg-[#FF3811]", // Custom background color for the popup
            title: "text-white font-bold", // Custom title styling
            confirmButton:
              "bg-[#444444] text-white px-6 py-3 rounded-lg hover:bg-[#FF3811] transition-all", // Custom button styling
          },
        });
        ...
        // Error notification for unsuccessful response
        console.error("Registration failed:", result); // Log the error message if provided
        Swal.fire({
          position: "center",
          icon: "error",
          title:
      ...
      <div className="lg:w-1/2">
        <div className="border-2 border-gray-300 p-8 sm:p-10 md:p-[75px] rounded-lg max-w-lg mx-auto">
          ...
          >
            ...
            >
              {loading ? (
                <AiOutlineLoading3Quarters className="w-5 h-5 mr-2 animate-spin" /> // Loading spinner
              ) : (
                "Sign Up"
              )}
            </button>
