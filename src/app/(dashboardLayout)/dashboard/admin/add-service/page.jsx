...
import Swal from "sweetalert2";
  ...
  const Page = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axiosSecure.get("/dashboard/admin/get-services");
    ...
    const onSubmit = async (data) => {
      ...
      if (data?.image && data.image.length > 0) {
        try {
          ...
          const response = await axiosSecure.post(process.env.NEXT_PUBLIC_IMAGEBBURL, formData, {
            headers: {
          ...
          try {
            const res = await axiosSecure.post("/dashboard/admin/add-service/api/post-service", newService);
            console.log(res);
            if (res?.status === 201 && res.data?.status === 201) {
              reset();  // Reset the form
              setServices((prev) => [...prev, newService]); // Add the new service to the state
